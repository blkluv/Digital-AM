export const siteCopy = {
  es: {
    meta: { title: "ViralDental.marketing | Sistemas digitales para consultorios dentales modernos" },
    ui: { languageLabel: "Idioma" },
    brand: {
      name: "ViralDental.marketing",
      tagline: "Estudio de sistemas digitales · Atlanta, Georgia",
      headerCta: "Reservar consulta",
    },
    nav: [
      { label: "Plataforma", href: "#platform" },
      { label: "Trabajos", href: "#work" },
      { label: "Contacto", href: "#contact" },
    ],
    hero: {
      eyebrow: "Estudio de marketing digital · Atlanta, Georgia",
      title: "Marketing digital para consultorios dentales modernos",
      subtitle: "Sitios web, automatización e infraestructura de leads diseñados para convertir atención en pacientes.",
      primaryCta: { label: "Reservar consulta", href: "https://calendly.com/viraldentalmarketing" },
      secondaryCta: { label: "Ver trabajos", href: "#work" },
    },
    system: {
      title: "Plataforma de crecimiento para pacientes",
      text: "Desde el momento en que un paciente ve tu anuncio hasta su primera visita y más allá, cada paso del recorrido debe sentirse sin esfuerzo.",
      flow: ["Anuncio pagado", "Publicación en redes", "Sitio web", "Captura de leads", "Automatización", "CRM", "Paciente"],
    },
    problem: {
      title: "La mayoría de los consultorios dentales pierden pacientes nuevos antes de que reserven",
      items: [
        {
          title: "Primera impresión falla",
          text: "Un sitio web desactualizado hace que los pacientes cuestionen tu calidad antes de que tengas oportunidad de impresionar.",
        },
        {
          title: "Fugas en el embudo",
          text: "Pacientes potenciales visitan tu sitio pero no llaman — y sin captura automatizada, se pierden para siempre.",
        },
        {
          title: "Respuesta lenta o nula",
          text: "Los leads tibios se enfrían cuando no se devuelven los mensajes de voz o las llamadas tardan horas en lugar de minutos.",
        },
      ],
    },
    platform: {
      eyebrow: "Plataforma Viral Dental Marketing",
      title: "Convierte más visitantes del sitio web en pacientes que pagan, automáticamente.",
      modules: [
        {
          title: "Sitios web que atraen pacientes",
          text: "Sitios construidos para generar confianza al instante, responder preguntas de los pacientes y hacer sonar el teléfono.",
        },
        {
          title: "Captura de leads que convierte",
          text: "Formularios inteligentes y click-to-text que capturan cada visitante, incluso cuando no llaman.",
        },
        {
          title: "Automatización de seguimiento instantáneo",
          text: "Secuencias de SMS y email 24/7 que responden en segundos y nunca dejan enfriar un lead.",
        },
        {
          title: "Pipeline de nuevos pacientes",
          text: "Un CRM construido para odontología que rastrea cada lead desde el primer clic hasta la primera visita.",
        },
      ],
    },
    work: {
      title: "Trabajos seleccionados",
      linkLabel: "Ver proyecto",
      items: [
        {
          title: "Zen Dentist",
          text: "Una experiencia digital tipo spa. El sitio utiliza un diseño minimalista con amplio espacio en blanco y tonos suaves para crear un entorno relajante y libre de estrés para pacientes nerviosos.",
          outcomes: ["Branding de dominio", "Experiencia mobile-friendly", "Navegación simple"],
          href: "https://www.zen.dentist/",
          status: "Publicado",
        },
        {
          title: "Relaxation Dental",
          text: "Integra expertamente la Optimización de Tasa de Conversión (CRO) en toda la experiencia de navegación. Utiliza un video de apertura inmersivo y CTAs estratégicos que refuerzan la promesa de marca.",
          outcomes: ["Presentación de servicios refinada", "Mayor confianza visual", "Mejor claridad de producto"],
          href: "https://www.relaxationdds.com/",
          status: "Publicado",
        },
        {
          title: "Staten Island Oral & Maxillofacial Surgery",
          text: "Prioriza la tecnología y la confianza. Mediante el uso de imágenes de alta calidad de las instalaciones y contenido educativo, ayuda a los visitantes a visualizar su procedimiento con claridad.",
          outcomes: ["Contenido educativo centrado en el paciente", "Comunicación de servicios más clara", "Experiencia mobile-friendly"],
          href: "https://statenislandoralsurgery.us/",
          status: "Publicado",
        },
      ],
    },
    technology: {
      title: "Anuncios que entregan. Software que hace seguimiento.",
      text: "Google, Facebook y TikTok traen nuevos pacientes a tu sitio. Nosotros nos aseguramos de que realmente reserven — con automatización moderna que funciona junto con Dentrix, Eaglesoft u Open Dental.",
      items: ["Anuncios Google/Facebook/TikTok", "Sitios web que convierten", "Recepcionista IA", "Captura de leads 24/7", "Seguimientos de texto instantáneos", "Mensajería bidireccional con PMS", "Automatización de recall", "Links de pago por texto"],
    },
    cta: {
      title: "Tu software trabaja más. Tú te enfocas en la odontología.",
      text: "Deja de luchar con herramientas obsoletas. Agregamos automatización moderna a lo que ya usas — Dentrix, Eaglesoft, Open Dental, lo que sea.",
      primaryCta: "Reservar consulta",
      email: "hola@viraldental.marketing",
      location: "Atlanta, Georgia",
    },
    contact: {
      formTitle: "Cuéntanos sobre tu negocio",
      fields: {
        name: { placeholder: "Nombre" },
        email: { placeholder: "Email" },
        message: { placeholder: "¿Qué necesitas resolver?" },
      },
      actions: { submit: "Reservar consulta", submitting: "Enviando...", direct: "Escríbeme directo" },
      messages: {
        incomplete: "Completa todos los campos antes de enviar.",
        success: "Enviado. Te responderé pronto.",
        localPreview: "Abre desde el sitio publicado.",
        fallback: "Envíame un correo directo a hi@viraldental.marketing.",
      },
    },
    footer: {
      copyright: "© {year} ViralDental.marketing.",
      credit: "Viral Dental Marketing",
    },
  },

  en: {
    meta: { title: "ViralDental.marketing | Digital systems for modern dental practices" },
    ui: { languageLabel: "Language" },
    brand: {
      name: "ViralDental.marketing",
      tagline: "Digital systems studio · Atlanta, Georgia",
      headerCta: "Book a consultation",
    },
    nav: [
      { label: "Platform", href: "#platform" },
      { label: "Work", href: "#work" },
      { label: "Contact", href: "#contact" },
    ],
    hero: {
      eyebrow: "Digital marketing studio · Atlanta, Georgia",
      title: "Digital marketing for modern dental practices",
      subtitle: "Paid marketing campaigns, website redesign, automation and lead infrastructure designed to convert attention into patients.",
      primaryCta: { label: "Book a consultation", href: "https://calendly.com/viraldentalmarketing" },
      secondaryCta: { label: "View work", href: "#work" },
    },
    system: {
      title: "Patient Growth Platform",
      text: "From the moment a patient sees your ad to their first visit and beyond, every step of the journey should feel seamless.",
      flow: ["Paid Ad", "Social Media Post", "Website", "Lead Capture", "Automation", "CRM", "Patient"],
    },
    problem: {
      title: "Most dental practices lose new patients before they ever book",
      items: [
        {
          title: "First impression fails",
          text: "An outdated website makes patients question your quality before you get a chance to impress.",
        },
        {
          title: "Leaks in the funnel",
          text: "Potential new patients visit your site but don't call — and without automated capture, they're gone forever.",
        },
        {
          title: "Slow or no response",
          text: "Warm leads go cold when voicemails aren't returned or follow-ups take hours instead of minutes.",
        },
      ],
    },
    platform: {
      eyebrow: "Viral Dental Marketing Platform",
      title: "Turn more website visitors into paying patients automatically.",
      modules: [
        {
          title: "Patient-Attracting Websites",
          text: "Sites built to build trust instantly, answer patient questions, and make the phone ring.",
        },
        {
          title: "Lead Capture That Converts",
          text: "Smart forms and click-to-text that capture every visitor, even when they don't call.",
        },
        {
          title: "Instant Follow-Up Automation",
          text: "24/7 SMS and email sequences that respond in seconds and never let a lead go cold.",
        },
        {
          title: "New Patient Pipeline",
          text: "A CRM built for dentistry that tracks every lead from first click to first visit.",
        },
      ],
    },
    work: {
      title: "Selected work",
      linkLabel: "View project",
      items: [
        {
          title: "Zen Dentist",
          text: "A spa-like digital experience. The site uses a minimalist, white-space-heavy layout and muted tones to create a calming, stress-free environment for nervous patients before they even book.",
          outcomes: ["+204 new patients YOY", "+68% recall appointment rate", "24/7 lead capture"],
          href: "https://www.zen.dentist/",
          status: "Live",
        },
        {
          title: "Relaxation Dental",
          text: "Expertly weaves Conversion Rate Optimization (CRO) throughout the scrolling experience. It uses an immersive opening video and strategically placed CTAs that reinforce the brand promise of a spa dental visit.",
          outcomes: ["-32% cost per lead", "+112% form completions", "Mobile-first design"],
          href: "https://www.relaxationdds.com/",
          status: "Live",
        },
        {
          title: "Staten Island Oral & Maxillofacial Surgery",
          text: "Prioritizes Technology & Trust. By using high-quality facility images and patient-centered educational content, it helps visitors visualize their procedure, which is critical for complex oral surgery.",
          outcomes: ["+53% surgical consults", "+87% online booking rate", "Referral-friendly experience"],
          href: "https://statenislandoralsurgery.us/",
          status: "Live",
        },
      ],
    },
    technology: {
      title: "Paid ads that deliver. Software that follows through.",
      text: "Google, Facebook, and TikTok bring new patients to your site. We make sure they actually book — with modern automation that works alongside Dentrix, Eaglesoft, or Open Dental.",
      items: ["Google/Facebook/TikTok ads", "Websites that convert", "AI receptionist", "24/7 lead capture", "Instant text follow-ups", "Two-way PMS texting", "Recall automation", "Text-to-pay links"],
    },
    cta: {
      title: "Ready to go viral?",
      text: "Stop fighting outdated tools. We add modern automation to whatever you already use — Dentrix, Eaglesoft, Open Dental, you name it.",
      primaryCta: "Book a consultation",
      email: "hi@viraldental.marketing",
      location: "Atlanta, Georgia",
    },
    contact: {
      formTitle: "Tell us about your business",
      fields: {
        name: { placeholder: "Name" },
        email: { placeholder: "Email" },
        message: { placeholder: "What do you need to solve?" },
      },
      actions: { submit: "Book a consultation", submitting: "Sending...", direct: "Email me directly" },
      messages: {
        incomplete: "Complete all fields before sending.",
        success: "Sent. I will reply soon.",
        localPreview: "Open from the published site.",
        fallback: "Email me directly at hi@viraldental.marketing.",
      },
    },
    footer: {
      copyright: "© {year} ViralDental.marketing.",
      credit: "Viral Dental Marketing",
    },
  },
};